...
use crate::json_rpc::{
    INTERNAL_ERROR, INVALID_PARAMS, JsonRpcResponse, MCP_PROTOCOL_VERSION, METHOD_NOT_FOUND,
};
...
impl McpHandler {
    pub fn new(iii: III, engine_url: String) -> Self {
    ...
    pub fn register(iii: &III) {
        ...
        iii.register_function(
            RegisterFunctionMessage {
                ...
                description: Some("MCP JSON-RPC handler — tools, resources, prompts".to_string()),
    ...
    async fn tools_list(&self) -> Result<Value, String> {
        ...
        if let Ok(functions) = self.iii.list_functions().await {
            tools.extend(functions.iter().map(function_to_tool));
        }
    ...
    async fn tools_call(&self, params: Option<Value>) -> Result<Value, String> {
        ...
        match params.name.as_str() {
            "iii_worker_register" => {
                ...
                match self.worker_manager.create_worker(p).await {
                    Ok(r) => return Ok(tool_json(&serde_json::to_value(&r).unwrap_or_default())),
                    Err(e) => return Ok(tool_error(&e)),
                }
            }
            "iii_worker_stop" => {
                let p: WorkerStopParams = parse(Some(params.arguments))?;
                match self.worker_manager.stop_worker(p).await {
                    Ok(r) => return Ok(tool_json(&serde_json::to_value(&r).unwrap_or_default())),
                    Err(e) => return Ok(tool_error(&e)),
                }
            }
            ...
            "iii_trigger_void" => {
                ...
                match self
                    ...
                    .await
                {
                    Ok(_) => return Ok(tool_result(&format!("Triggered (void): {}", fid))),
                    Err(e) => return Ok(tool_error(&format!("Error: {}", e))),
                }
            }
            "iii_trigger_enqueue" => {
                let fid = str_field(&params.arguments, "function_id");
    ...
    fn resources_list(&self) -> Value {
        json!({
            "resources": [
                { "uri": "iii://functions", "name": "Functions", "description": "All registered iii-engine functions", "mimeType": "application/json" },
                { "uri": "iii://workers", "name": "Workers", "description": "Connected workers with metrics", "mimeType": "application/json" },
                { "uri": "iii://triggers", "name": "Triggers", "description": "Registered triggers", "mimeType": "application/json" },
                { "uri": "iii://context", "name": "Context", "description": "Runtime capabilities: Logger, State, Events, Streams, Channels", "mimeType": "application/json" },
            ]
        })
    }
            ...
            "iii://context" => (
                serde_json::to_string_pretty(&json!({
                    ...
                    "capabilities": {
                        ...
                        "state": { "functions": ["state::get", "state::set", "state::delete", "state::update", "state::list"] },
                        "events": { "functions": ["emit"] },
                        "trigger_actions": { "modes": ["sync", "void", "enqueue"] },
                        "streams": { "functions": ["streams::get", "streams::set", "streams::delete", "streams::update", "streams::list"] },
                        "channels": { "functions": ["engine::channels::create"] }
                    }
                }))
                .unwrap(),
                "application/json",
...
fn initialize_result() -> Value {
    json!({
        ...
        "serverInfo": { "name": "iii-connect", "version": env!("CARGO_PKG_VERSION") },
        "instructions": "iii-engine MCP server (SDK v0.9). Function IDs use :: delimiter."
    })
}
    ...
    let result = match method {
        ...
        "tools/list" => {
            ...
            if let Ok(fns) = iii.list_functions().await {
                tools.extend(fns.iter().map(function_to_tool));
            }
            Ok(json!({ "tools": tools }))
        }
        "tools/call" => {
        ...
        "prompts/list" => Ok(super::prompts::list()),
        "prompts/get" => Ok(super::prompts::get(params)),
        _ => Err(format!("Unknown method: {}", method)),
    };
...
fn builtin_tools() -> Vec<McpTool> {
    vec![
        McpTool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "code": { "type": "string", "description": "Async handler function code" },
                    "function_name": { "type": "string", "description": "Function ID (:: delimiter)" },
        ...
        McpTool {
            ...
            description: Some("Stop a worker and clean up".into()),
            input_schema: json!({ "type": "object", "properties": { "id": { "type": "string" } }, "required": ["id"] }),
        },
        McpTool {
            name: "iii_trigger_register".into(),
            description: Some("Register a trigger (http, cron, queue) for a function".into()),
            input_schema: json!({
                "type": "object",
                "properties": { "trigger_type": { "type": "string" }, "function_id": { "type": "string" }, "config": { "type": "object" } },
        ...
        McpTool {
            ...
            input_schema: json!({
                "type": "object",
                "properties": { "function_id": { "type": "string" }, "payload": { "type": "object" } },
                "required": ["function_id", "payload"]
            }),
        },
        McpTool {
            ...
            input_schema: json!({
                "type": "object",
                "properties": { "function_id": { "type": "string" }, "payload": { "type": "object" }, "queue": { "type": "string" } },
                "required": ["function_id", "payload", "queue"]
            }),
        },
    ]
}
